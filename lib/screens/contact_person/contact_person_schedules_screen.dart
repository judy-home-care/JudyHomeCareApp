...
class ContactPersonSchedulesScreen extends StatefulWidget {
  ...
  const ContactPersonSchedulesScreen({
    Key? key,
    required this.patientId,
  }) : super(key: key);

  @override
  ContactPersonSchedulesScreenState createState() =>
      ContactPersonSchedulesScreenState();
}
...
class ContactPersonSchedulesScreenState
  ...
  Future<void> _loadSchedules(
    ...
    if (!forceRefresh &&
        _scheduleCache.containsKey(cacheKey) &&
        !_isCacheExpired) {
      if (mounted) {
        setState(() {
    ...
    try {
      ...
      final schedules = await _contactPersonService.getSchedules(
        date: selectedDateStr,
        status: _selectedFilter == 'All Shifts' ? null : _selectedFilter,
      );
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed:
                _isLoading ? null : () => _loadSchedules(forceRefresh: true),
            tooltip:
          ...
          Container(
            ...
            child: Row(
              children: [
                Expanded(
                  child:
                      _buildTabButton('Scheduled', 'upcoming', _upcomingCount),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: _buildTabButton(
                      'Completed', 'completed', _completedCount),
                ),
                const SizedBox(width: 12),
                Expanded(
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                if (_scheduleCache.containsKey(_getCacheKey()) &&
                    !_isCacheExpired)
                  Tooltip(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                _buildShimmerBox(width: 4, height: 50, borderRadius: 2),
                const SizedBox(width: 12),
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Row(
                        children: [
                          Expanded(
                            child:
                                _buildShimmerBox(height: 18, borderRadius: 8),
                          ),
                          const SizedBox(width: 12),
                          _buildShimmerBox(
                      ...
                      const SizedBox(height: 8),
                      _buildShimmerBox(width: 150, height: 14, borderRadius: 6),
                    ],
                  ),
                ),
              ],
                  ...
                  children: [
                      ...
                      [
                        ...
                        if (schedule.actualStartTime != null)
                          _buildDetailItem(
                            'Actual Start',
                            DateFormat('h:mm a')
                                .format(schedule.actualStartTime!),
                          ),
                    ...
                    if (schedule.carePlanTitle != null &&
                      ...
                      _buildDetailSection(
                        ...
                        [
                          _buildDetailItem(
                              'Plan Name', schedule.carePlanTitle!),
                        ],
                      ),
                    ],
                    if (schedule.notes.isNotEmpty) ...[
                    ...
                    // Request Reschedule button (only for non-completed/cancelled schedules)
                    if (!schedule.isCompleted &&
                        schedule.status != 'cancelled' &&
                        schedule.status != 'in_progress' &&
                        !schedule.hasPendingRescheduleRequest) ...[
                      SizedBox(
                        width: double.infinity,
                      ...
                      Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                'A reschedule request is pending review',
                                style: TextStyle(
                                  fontSize: 13,
            ...
            if (_selectedFilter != 'All Shifts' ||
              ...
              OutlinedButton.icon(
                ...
                style: OutlinedButton.styleFrom(
                  ...
                  side: BorderSide(
                      color: _primaryColor.withOpacity(0.5), width: 2),
                  padding:
                  ...
                  child: ElevatedButton(
                    onPressed: isSubmitting || selectedReason == null
                        ...
                        : () async {
                            ...
                            try {
                              final response = await _contactPersonService.requestReschedule(
