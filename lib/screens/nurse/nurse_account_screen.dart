...
import 'nurse_time_logs_screen.dart';
import '../care_plans/care_plan_history_screen.dart';
import 'nurse_certifications_screen.dart';
...
class NurseAccountScreen extends StatefulWidget {
  ...
  const NurseAccountScreen({
    Key? key,
    required this.nurseData,
  }) : super(key: key);

  @override
  State<NurseAccountScreen> createState() => _NurseAccountScreenState();
}
...
class _NurseAccountScreenState extends State<NurseAccountScreen>
  ...
  Widget _buildAppBar() {
    // Extract initials from nurse name
    String getInitials(String name) {
      List<String> nameParts = name.trim().split(' ');
      if (nameParts.isEmpty) return 'NU';
      if (nameParts.length == 1) return nameParts[0][0].toUpperCase();
      return '${nameParts[0][0]}${nameParts[nameParts.length - 1][0]}'.toUpperCase();
    }

    final avatarPath = _currentNurseData['avatar'];
          ...
          child: SafeArea(
            child: Column(
              ...
              children: [
                        ...
                        ? Image.network(
                            ...
                            errorBuilder: (context, error, stackTrace) {
                              ...
                              return _buildInitialsAvatar(getInitials(
                                _currentNurseData['name'] ?? 
                                _currentNurseData['full_name'] ?? 
                                'Nurse User'
                              ));
                            },
                ...
                Text(
                  _currentNurseData['name'] ?? 'Nurse Name',
                ...
                Text(
                  _currentNurseData['email'] ?? 'nurse@email.com',
                  style: TextStyle(
                    fontSize: 14,
      ...
      child: Column(
        ...
        children: [
          ...
          _buildSettingTile(
            ...
            iconColor: const Color(0xFF2196F3),
            iconBg: const Color(0xFFE3F2FD),
            onTap: () => _navigateToTimeLogs(),
          ),
          _buildDivider(),
          // _buildSettingTile(
          //   icon: Icons.history,
          //   title: 'Care Plan History',
          //   subtitle: 'View your patient care records',
          //   iconColor: const Color(0xFF9C27B0),
          //   iconBg: const Color(0xFFF3E5F5),
          //   onTap: () => _navigateToCarePlanHistory(),
          // ),
          _buildDivider()
        ],
      ),
  ...
  void _navigateToTimeLogs() {  
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => NurseTimeLogsScreen(
          nurseData: _currentNurseData,
        ),
      ),
    );
  }

  void _navigateToCarePlanHistory() {  
    Navigator.push(
      context,
      MaterialPageRoute(
  ...
  void _navigateToFAQs() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => const NurseFAQScreen(),
      ),
    );
  }
  ...
  void _navigateToHelpCenter() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => const NurseHelpCenterScreen(),
      ),
    );
  }
  ...
  Future<void> _navigateToPersonalInfo() async {
    ...
    if (updatedData != null && mounted) {
      ...
      // Update the current nurse data
      setState(() {
        _currentNurseData = updatedData;
      });
      
      print('🔄 Account screen UI refreshed with new profile data');
      print('   New Name: ${_currentNurseData['name']}');
      print('   New Email: ${_currentNurseData['email']}');
    } else {
